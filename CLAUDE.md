...
- **LTX video: native, production, the default route. ✓ (migration complete — verified live, committed.)** LTX renders an **audio+video** clip through the real app's native path: `run_native_video` → open gate → `run_native_split_stack_video` → `LtxVideoAdapter` → `_build_native_ltx_video_prompt()` → ComfyUI `/prompt`. `backend_route="native_comfy_template"`, contract `validation_status="production"`. The old Prompt-API hard-route (passes 29F/29H2/29I + policy promotion) is **removed**; routing is by `resolved_native_video_family`, not substring matching. The UI labels LTX as a native/validated family (no more UI-vs-backend divergence). The explicit `ltx_prompt_api_gated_submission` command + its engine are **kept as a dead-but-available fallback** (history-requeue + safety net), reachable only by explicit opt-in. **Both t2v and i2v are proven native** — i2v runs through a keyframe-upload bridge (`_upload_comfy_image` → ComfyUI `/upload/image` → `LoadImage`, then `LTXVImgToVideoConditionOnly` with `bypass=False`); verified live that frame 0 pins to the input keyframe (MAE 3.5) and motion evolves after. (The i2v carve-out is LTX-only; Wan i2v stays blocked.)
  - **The template** is `python/video_templates/ltx_av_native.json` — a repo-owned 31-node AV graph, pruned by reachability from the proven `ltx_api.json` (dual-pass → single AV output; the dropped Pass-B was the distilled `ManualSigmas` path). The builder patches user inputs into it (prompt, dims, length, fps, seed, steps→`LTXVScheduler`, cfg→VIDEO guider `4964`, lora→`4968`, t2v/i2v→bypass flag `4977` + image `2004`, asset names). Source node IDs/inputs from a **live `/object_info` dump**, never from memory — this is what made the migration go clean (the `4960.ckpt_name` missing-`ltx\`-prefix bug was caught this way).
  ...
  - **LTX quality & VRAM (from the native smoke test + community configs):**
    ...
    - **The 31.4 GB peak is the FULL-PRECISION 22B path** (`ltx-2.3-22b-dev`, no quantization, no offload, AV with **both** VAEs resident). Community configs report comfortable headroom at equal/larger frames on 24 GB cards — but those run **FP8 / NVFP4 quantized or distilled** variants and/or **model offload**, not the full fp16/bf16 22B. So our number is plausible for the heaviest config, and there is likely **large headroom** via (a) an **FP8/NVFP4 quantized** LTX checkpoint, or (b) **model offload** (transformer→RAM during VAE decode). **Open question (later pass):** is a quantized `ltx-2.3-22b` variant present under `D:/AI_ASSETS`, and does ComfyUI's LTX node set support offload here? Either could unlock higher native resolution (the real softness fix) within 32 GB. **Do not change the smoke-test config now** — future optimization thread, after the migration lands.
  - **`_comfy_object_info` hardened** (Connection: close + 5× retry + 90s timeout, raises on exhaustion never a partial dict) — the ~2MB `/object_info` body could reset mid-read as a `ConnectionResetError` (an `OSError`, not a `urllib.URLError`, so the old single `urlopen` slipped it through and aborted native video gen).
...
- **Models page — REAL, now wired.** The "Models" rail entry hosts **`ModelManagerPage`** (`ModelManagerPage.{h,cpp}` — note: **not** `ModelsPage`, which doesn't exist), the **Stage-1 model-inventory browser**: recursive scan of the models root (`D:/AI_ASSETS/models`), a Name/Type/Family/Size/Status tree with family+type detection, a details panel, and disk caching (`model_inventory_cache.json`). Verified live **2026-07-12**: 712 assets, 9 families, live scan. It is **not** the full 552-line `SPELLVISION_MODEL_MANAGER_SPEC.md` (no downloads [cache root "not configured"], no dependency health, no compatibility cues) — Stage 1 only. **History note:** it was orphaned Apr–Jul 2026 — commit `b4e1d6b` ("Checkpoint before Sprint 13 Recovery Pass 3") swapped it for a `ModePage` stub during a recovery checkpoint and it was never re-wired until the page-type restore (`fix(ui): restore the real ModelManagerPage`). It kept compiling the whole time (in `CMakeLists`, maintained through the spacing sweep). Its widgets **predate** the theme-token/typography/spacing sweeps, so it renders flatter than sibling surfaces (plain stacked header labels, a basic tree) — functional and themed via the global palette, but a visual-polish debt.
- **Placeholder pages:** **Inspire** still renders as a generic `ModePage` "Planned Section / Coming Soon" stub. (Models is no longer a stub — see the line above.)
- **Flows / Workflow Library:** works — the built list/detail/readiness UI populates with real imported workflows (verified live **2026-07-06**: 2 imported, `ltx-api-json` + `nest-json`; header shows "Workflows: 2 · Ready: 1 · Missing Dependencies: 1"; import is functional). The earlier "0 imported workflows" was a transient empty-`runtime/imported_workflows` state, since populated by real imports (`ltx-api-json` from the LTX migration, `nest-json`) — **not a bug and not an asset-root issue.**
...
4. **RESOLVED — LTX is native/production.** The UI-vs-backend divergence is gone: native LTX migration complete (template + adapter + builder, redirects removed, contract flipped to production, UI labels reconciled). LTX renders AV through `native_comfy_template`; Prompt-API engine kept as explicit fallback only. Both t2v and i2v are proven native. See §6.
