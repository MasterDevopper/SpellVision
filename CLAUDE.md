...
**The Rust core is gone.** An early vestigial Rust job-queue stub (`spellvision_rust` / `spellvision_core`) was archived to `attic/rust_original_intent/` and unwired from `CMakeLists.txt`. Nothing in C++ references it. **The stale prerequisite lists are already purged (verified 2026-09-02):** `docs/SPELLVISION_DEV_GUIDE.md`'s Prerequisites section names Qt 6, Python 3.12+, CMake and VS Build Tools and no Rust; `docs/SPELLVISION_ARCHITECTURE.md` states there is no Rust in the live build. The only remaining mentions are records of the archival, which is what they should be.
...
- **Phase D — Hard frontiers (last).** Native **Text-to-3D** / **Image-to-3D** adapters are not started. `Gen3DPage` exists as a **hidden** Comfy-workflow passthrough. **Dataset generation** is on the rail (`dataset`) and talks to the worker — not “integrate an existing standalone app” anymore. **Character B (mesh / garments / hair / beauty) is CUT TO v2 — owner decision 2026-09-04**, superseding the 2026-08-17 lock that put it in the v1 bar; the stated reason is that it serves the owner's own workflow rather than public intent, and a ship gate held for one user's workflow is the wrong shape for this release. Character Studio stays on the rail and stays working — what is cut is the product bar, not the surface. **3D is also v2, and the direction changed: TRELLIS.2 rather than Hunyuan3D-2.1 first** (MIT vs a community licence, and thin/transparent geometry rather than one fused watertight mesh). Re-survey before building — see the churn warning below.
...
- **LTX video: native, production. Default route = `two_stage_distilled`** (`native_video_graphs._ltx_route_for`). Opt in to single-stage-full with `ltx_route='single_stage_full'`. LTX renders audio+video through `run_native_video` → `LtxVideoAdapter` → ComfyUI `/prompt`. Contract `validation_status="production"`. Prompt-API is **history-requeue / explicit opt-in only**. Both t2v and i2v are proven native — i2v uses a keyframe-upload bridge (`_upload_comfy_image` → `LoadImage` → `LTXVImgToVideoConditionOnly`); live frame-0 MAE ~3.5.
  ...
  - **LTX quality & VRAM (from the native smoke test + community configs):**
    ...
    - **VRAM / resolution — CORRECTED 2026-09-03. The old text here said 768×512×97f peaked ~31.4/32 GB, that higher resolution "will OOM at 97 frames on 32 GB", that LTX is a "premium, near-ceiling" path, and that Simple-mode defaults must cap res×frames. All four were read off a number that was never a requirement.** Core v0.34.0 runs **DynamicVRAM (`comfy-aimdo`), on by default**, hooking CUDA allocation directly (`cuda-detour.c`, 6 hooks) and paging weights to host RAM under NVML pressure. It expands to fill whatever VRAM it is given, so **a peak figure measures what was AVAILABLE, not what was needed.** Measured with `--reserve-vram` shrinking the budget, seeds varied so no run hit the node cache:

      | reserve | budget | 768×512×49f | 768×512×97f | 1024×640×97f |
      |---|---|---|---|---|
      | none | ~31.8 GB | 31.39 GB / 79.7 s | 30.50 GB / 98.6 s | 30.98 GB / 160.1 s |
      | 16 GB | ~15.8 GB | 17.49 GB / 73.3 s | — | — |
      | 22 GB | ~9.8 GB | **11.47 GB** / 89.4 s | **11.50 GB** / 112.6 s | **11.47 GB** / 140.6 s |

      The full-precision 22B path renders **97 frames at 2048×1280 output in 11.47 GB**, for a **12–14%** time cost; at a ~16 GB budget the cost is not measurable. **VRAM is not LTX's constraint on this core — host RAM is:** the same renders peaked at **59.5 GB of 61.7 GB**, about **26–27 GB added by the render and near-identical at both VRAM budgets**, and nothing in the product measures it. A res×frames cap justified by VRAM is unfounded; a *host-RAM* requirement is real and unstated. **Generate at target resolution; do not upscale** still holds (it is a quality claim, not a memory one). See `python/comfy_launch_policy.py`.
...
- **Character Studio + Comic Studio + Concept Lab — REAL, on rail.** `qt_ui/studios/CharacterStudioPage` / `ComicStudioPage` / `ConceptReferencePage`; modeIds `character` / `comic` / `concept`. Generation handoff through existing T2I/I2I path. Design: `docs/design/29_character_comic_studios.md`, `31_concept_reference_lab.md`. Look/clothes commands (`look_complete`, `clothes_only`, `garment_shrinkwrap`) are wired. **Character B** (mesh / garments / hair / beauty product gates) is **CUT TO v2** (owner, 2026-09-04) and is no longer a v1 ship gate. The studios themselves ship. The original observation stands as the reason it was never close: UI-exists ≠ product-complete — mesh absent, hair zero on both sides, House LoRA never trained.
...
- ComfyUI — execution backend (LIVE = `C:\sv_comfynext_v034\ComfyUI`, isolated venv, post-2026-08-31 cutover; `C:\sv_comfynext\ComfyUI` and the D: build are rollback — see §9.2)

**Documentation (real names — there is no numbered "Codebook").** Start here: `README.md`, `CLAUDE.md`, `brain/00 Home.md`, `docs/SPELLVISION_WORKER_PROTOCOL.md`, `docs/design/27_v1.0_task_backlog.md`, `docs/design/28_release_readiness_checklist.md`, `docs/design/SpellVision_v1.0_Roadmap.md`, `docs/design/16_theme_token_reference.md`. `docs/SPELLVISION_FEATURE_MATRIX.md` is a map — prefer code + Current State Ledger when they conflict. **Corrected 2026-09-02:** this section used to say seven historically cited files "are not in tree". All seven exist — `ARCHITECTURE.md` at the root, and `SPELLVISION_DEV_GUIDE.md`, `SPELLVISION_INSTALLATION_GUIDE.md`, `SPELLVISION_MODEL_MANAGER_SPEC.md`, `JOB_LIFECYCLE_CONTRACT.md`, `PINNED_VENV_COMFY_README.md`, `DEV_WORKFLOW.md` under `docs/`. They were cited by their bare names, looked for at the root, and written off. `docs/sprints/*` are historical pass READMEs — intent, not current truth.

**Present-tense documents** — the ones that must describe the tree as it is now — are `CLAUDE.md`, `README.md`, `ARCHITECTURE.md`, `docs/design/50_feature_build_standard.md` and `docs/design/53_consistency_and_robustness_audit.md`. `ARCHITECTURE.md` is the operations map: the three processes, the Generate path end to end, the module tables, how the test lanes / sweeps / hook / CI / ctest fit together, and how to add a command, a family, a rule or a C++ test. `tests/test_governing_docs.py` asserts every source file those five name actually exists — it exists because `ARCHITECTURE.md` was routing readers to four modules deleted four passes earlier.
...
1. **RESOLVED — Asset / model root = `D:/AI_ASSETS/`** (user-confirmed; models, checkpoints live there). **(NOTE: after the 2026-08-31 cutover the ComfyUI *install* is `C:\sv_comfynext_v034\ComfyUI` — see §9.2 — but MODELS/checkpoints/assets stay `D:/AI_ASSETS/models`, shared into the new install via the copied `extra_model_paths.yaml`.)** The `.env` (`${SPELLVISION_ROOT}/models`) and `runtime_paths.py` (`external_assets/`) values are drift to reconcile against this, not alternatives. **Exception:** the imported-workflow library root is deliberately `<projectRoot>\runtime\imported_workflows` — project-relative on C:, by design (verified populated + working **2026-07-06**; Flows is **not** a §9 audit target — the earlier "0 workflows" was a stale empty-dir state, see §6).
2. **RESOLVED (2026-08-31 cutover, Doc 25 S7) — LIVE ComfyUI = `C:\sv_comfynext_v034\ComfyUI`** (Comfy-Org core `12d5279`, **v0.34.0**, 2026-08-25), run from the **ISOLATED venv `C:\sv_comfynext_v034\.venv`** (kornia **pinned 0.8.2** + sageattention 1.0.6 / triton-windows 3.7.1.post27; **`PYTHONUTF8=1` required**), on **:8188** (worker :8765 unchanged, project `.venv`, venvs decoupled). Cutover was a config repoint: `comfy_root.LIVE_COMFY`, `RuntimeProfile.cpp::kLiveComfyRoot`, and the two launchers. **Rollback is now TWO-DEEP and ordered newest-first** — `C:\sv_comfynext\ComfyUI` (v0.27.0, Jul-10 core `206b9245`) then `D:\AI_ASSETS\comfy_runtime\ComfyUI` (cf9cbec5, May) (+ `F:\comfy_backup`) — both listed in `SUPERSEDED_COMFY` / `kSupersededComfyRoots`, which is what makes `prefer_live` redirect a stored path carrying ANY former root instead of only the oldest. **Do NOT edit/probe a superseded tree as live.** `ComfyUI-LTXVideo` must be ≥ `15d09ab` on this core (`aceeae9` fails to import). `ComfyUI-MagCache` is removed — upstream is a dead end against the v0.34.0 rope change. `runtime_paths.py:default_comfy_root()` (`runtime/comfy/ComfyUI`) is unused drift. See `docs/pipeline/comfy_baselines/README.md` for the render gate.
...
5. **RESOLVED (verified 2026-09-02) — no doc lists Rust as a prerequisite.** `docs/SPELLVISION_DEV_GUIDE.md` and `docs/SPELLVISION_ARCHITECTURE.md` were both already correct; this entry outlived the purge it asked for by several passes. The lasting fix is `tests/test_governing_docs.py`, which holds the five present-tense documents to the tree — the audit line that says "purge X" is itself a claim that can go stale, and this one did.
