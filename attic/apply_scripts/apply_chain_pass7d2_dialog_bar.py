r"""
SpellVision — Chain Studio Pass 7d.2: top dialog bar.

Three coordinated edits:

1. CMakeLists.txt — register ChainDialogBarWidget.h/.cpp.

2. ChainStudioPage.h — forward-declare ChainDialogBarWidget; add
   dialogBarWidget_ member; add three slot declarations for the new
   dialog-bar signals (inputImageSelected, addStageRequested,
   promptChanged).

3. ChainStudioPage.cpp — full-file rewrite. The buildTopStrip()
   placeholder is replaced with a real ChainDialogBarWidget bound to
   stubChain_. The widget's signals route to stub handlers that mutate
   the stub chain (entry image + prompt + add stage). The rail's
   addStageRequested still routes to the same stub handler as the
   dialog bar's, since 7d.3 will unify both via the kind-picker menu.

Reuse-first: the dialog bar uses ThemeManager tokens for all colors /
spacing / radii, mirrors the v3 mockup's .dialog-line / .upload /
.dbar / .addbtn structure, and integrates QFileDialog the same way
ImageGenerationPage does.

Idempotent. Full-file rewrite of ChainStudioPage.cpp.
"""
...
MARKER = "CHAIN STUDIO PASS 7D2 DIALOG BAR"
CMAKE_BACKUP = ".pre_pass7d2_cmake.bak"
HDR_BACKUP   = ".pre_pass7d2_page_hdr.bak"
CPP_BACKUP   = ".pre_pass7d2_page_cpp.bak"
...
# =============================================================================
# 1. CMakeLists.txt — append after 7D1's block
# =============================================================================
...
def patch_cmake(project: Path) -> None:
    ...
    text = replace_once(text, CMAKE_ANCHOR, CMAKE_REPLACEMENT,
                        "Pass 7D1 CMake block tail")
    write_text(path, text)
    print(f"  Patched: {path.name}")


# =============================================================================
# 2. ChainStudioPage.h — forward-decl + member + slots
# =============================================================================
...
HDR_FWD_REPLACEMENT = (
    "// --- CHAIN STUDIO PASS 7D1 CONFIG PANEL: forward-declare ChainConfigPanelWidget ---\n"
    "class ChainConfigPanelWidget;\n"
    f"// --- {MARKER}: forward-declare ChainDialogBarWidget ---\n"
...
def patch_page_header(project: Path) -> None:
    ...
    text = replace_once(text, HDR_MEMBER_ANCHOR, HDR_MEMBER_REPLACEMENT,
                        "config-panel member tail")
    write_text(path, text)
    print(f"  Patched: {path.name}")


# =============================================================================
# 3. ChainStudioPage.cpp — full rewrite with dialog bar integrated
# =============================================================================
...
def main() -> int:
    ...
    print()
    print("Save ChainDialogBarWidget.h/.cpp to qt_ui/chain/ first, then:")
