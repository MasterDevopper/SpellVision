...
### Pass C2 — `req`-mutation ownership *(the refactor spine; behavior-preserving)*
...
### Pass C3 — Command-identity normalization *(behavior-preserving)*
- **Goal:** collapse the 6 aliased keys (`command`/`worker_command`/`execution_command`/`dispatch_command`/`task_command`/`workflow_task_command`) to one canonical field with a single read accessor, before any dispatcher extraction.
- **Scope:** introduce `canonical_command(req)` as the sole reader; keep writing the 6 keys *for now* (protocol/back-compat) but route every internal switch through the accessor. Removing the redundant keys from the wire is a later, separately-gated protocol change (out of this pass — flagged).
- **Behavior:** preserving. **Gate:** every existing dispatch decision resolves identically (characterization test replays a captured set of requests through `canonical_command` and asserts the same `run_*` target). **Pairs with C1** (both touch the dispatch seam) — may ship together or C3 first.
...
| 1 | **C3: command-identity accessor** | preserving | — | replay dispatch decisions identical | `pre-c3` |
...
| 3 | **C2: `req` ownership (B1)** | preserving | — (parallel to C1/C3) | builder-graph baseline byte-identical **+ exhaustive `req` key-set-diff matches per-path baseline** (G-graph doesn't cover `req` state) | `pre-c2` |
...
| **G-dispatch** (new) | C1 (#2), C3 (#1), dispatch (#12) | replay a captured set of requests (t2i/i2i/t2v/i2v/comfy_workflow/ltx_gated/noop_slow) through the dispatcher, assert the same `run_*` target + the same native-image-fork decision |
