...
| **Territorial notice (Hunyuan)** | The Tencent Hunyuan Community Licence excludes the **European Union, United Kingdom and South Korea** from its Territory, and no other family carries a restriction of that shape. **DECIDED 2026-09-04 — ship with a notice, dismissible.** The user can tick "don't show this again"; the notice states the excluded territories rather than a vague "restricted licence", because a warning nobody can act on is the same as no warning. Weights are not bundled (§0), so this is disclosure, not a distribution question. **Build task, not yet implemented** — it rides the existing badge/warn plumbing from the row below, which already derives from `model_registry` rather than naming a family in C++. |  dev | |
...
| Gate | Assertion | Owner | Evidence |
|---|---|---|---|
| Pinned stack scanned | torch 2.10+cu128 / kornia 0.8.2 / sageattention / triton-windows recorded + CVE-scanned at ship | dev | **SEQUENCED 2026-09-04 (owner):** a full codebase review plus the visual polish/refinement pass comes FIRST, then the CVE scan. Right order — a scan is a point-in-time claim about a pinned set, so running it before a review that may still move pins produces a result that expires before ship. |
...
- [x] **Model tiers 2–4** — **DECIDED 2026-09-04.** Tiers 3 and 4 were already built AND wired (`WorkflowLibraryPage.cpp` sends `resolve_missing_models`; `ModelResolutionDialog` shows the ranked offers), so the undecided part was only the Note-node question. **Owner: build it as a HINT.** 8 of 81 workflows carry model URLs as prose in a Note node (28 URLs); nothing links that text to a filename widget, so it is offered for confirmation and can never resolve on its own — Doc 19's rule. The measured reason that line stays hard: tier 3 written as a name search "resolved" 9 of 10 real missing checkpoints and the top hit for **4 of them was a style LoRA, not the checkpoint**; constrained to `types=Checkpoint` + exact filename it answers 5 of 10, all correct. A search that answers 9 and is wrong about 4 is worse than one that answers 5 and knows it. Original note kept for the record: Tier 0/1 ship (present locally, workflow-declared URL). A workflow naming a model that is absent and undeclared currently reports "the workflow names this model but gives no source", which is honest but leaves the user to find it. Decide before sign-off whether that is acceptable for v1. **Note:** the planned "exact identity via hash/AIR" tier was dropped — measured across all 81 workflows, **0** carry an AIR identifier and **0** carry a model hash; the "~12%" in the plan was a substring false positive (`air` inside "hair"/"chair"). See Doc 46 §9.
...
- [x] **Character B (mesh / garments / hair / beauty) — CUT TO v2. Owner decision 2026-09-04**, superseding the 2026-08-17 lock that put it in the v1 bar. Stated reason: *"Character B is more for my personal use than public intent."* A feature carried as a ship gate for a single user's workflow is the wrong shape for a release whose promise is ComfyUI-class generation without node graphs. **This removes the largest genuinely-unbuilt item from v1** — mesh absent, hair zero on both sides, House LoRA never trained, and a two-repo gate. Character Studio itself STAYS on the rail and stays working: what is cut is the mesh/garment/hair/beauty product bar, not the surface.
- [x] **3D — CUT TO v2, and the direction changed.** Owner 2026-09-04: *"a more general TRELLIS.2 implementation makes more sense for broader general user scope."* That supersedes Doc 46/CLAUDE.md §5's Hunyuan3D-2.1-first plan, and on two grounds that were already recorded there: TRELLIS.2 is **MIT-licensed** where Hunyuan3D is a community licence (and §9's Hunyuan territorial exclusion now makes that difference concrete), and it produces thin/transparent geometry rather than one fused watertight mesh. **Re-survey before building** — CLAUDE.md §5 warns the 3D landscape churns roughly every six months and that the chosen model is a swappable component.
- [x] **Character / Comic as v2.0-only** — partially reinstated: the STUDIOS ship (on rail, generation handoff works); the Character *product* bar does not. See the row above.
